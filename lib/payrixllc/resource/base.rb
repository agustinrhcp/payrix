  ...
  module Resource
    class Base
      ...
      def initialize(params, attrs)
        @attrs = attrs
        @request_options = Payrixllc::Http::RequestParams.new
      ...
      def retrieve(params = {})
        ...
        method = 'get'
        url = Payrixllc.configuration.url
        endpoint = "#{@resource_name}?#{query_params.join('&')}"
        data = {}

        body, status = Payrixllc::Http::Request.instance.send_http(method, url, endpoint, data, headers)
        @response = Payrixllc::Http::Response.new(body, status, self.class)
      ...
      def update(params = {})
        ...
        if !id 
          if Payrixllc.configuration.exception_enabled
            raise Payrixllc::Exceptions::InvalidRequest.new('ID is required for this action')
          else
            return false
          end
        end
      ...
      def delete(params = {})
        ...
        if !id 
          if Payrixllc.configuration.exception_enabled
            raise Payrixllc::Exceptions::InvalidRequest.new('ID is required for this delete')
          else
            return false
          end
        end
    ...
    protected
      def build_headers()
        config = Payrixllc.configuration

        if !config.url
          raise Payrixllc::Exceptions::InvalidRequest.new('Invalid URL')
        end
      ...
      def validate_response
        if @response.has_errors?
          if Payrixllc.configuration.exception_enabled
            raise Payrixllc::Exceptions::ApiError.new('There are errors in the response')
          end
